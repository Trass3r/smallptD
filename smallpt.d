...
struct Vec
{
	...
	Vec opMul(double b) const { return Vec(x*b,y*b,z*b); }
...
unittest
{
	Vec zero, one = Vec(1,1,1), v1=Vec(-1,0,2), v2=Vec(10,-20,5.5);
	assert(zero+one == one);
	assert(zero*one == zero);
	assert(v1+v2 == Vec(9,-20,7.5));
	assert(v1-v2 == Vec(-11,20,-3.5));
	assert(v1.dot(v2) == 1);
	assert(v1*v2 == Vec(-10,-0.,11));
	v1.norm();
	auto t = v1 - Vec(-0.44721359549995793928183473374626, 0, 0.89442719099991587856366946749251);
...
__gshared const Sphere[9] spheres = [ //Scene: radius, position, emission, color, material
	Sphere(1e5, Vec( 1e5+1,40.8,81.6),	Vec(),			Vec(.75,.25,.25),	Refl.DIFF),//Left
	Sphere(1e5, Vec(-1e5+99,40.8,81.6),	Vec(),			Vec(.25,.25,.75),	Refl.DIFF),//Rght
	Sphere(1e5, Vec(50,40.8, 1e5),		Vec(),			Vec(.75,.75,.75),	Refl.DIFF),//Back
	Sphere(1e5, Vec(50,40.8,-1e5+170),	Vec(),			Vec(),				Refl.DIFF),//Frnt
	Sphere(1e5, Vec(50, 1e5, 81.6),		Vec(),			Vec(.75,.75,.75),	Refl.DIFF),//Botm
	Sphere(1e5, Vec(50,-1e5+81.6,81.6),	Vec(),			Vec(.75,.75,.75),	Refl.DIFF),//Top
	Sphere(16.5,Vec(27,16.5,47),		Vec(),			Vec(1,1,1)*.999,	Refl.SPEC),//Mirr
	Sphere(16.5,Vec(73,16.5,78),		Vec(),			Vec(1,1,1)*.999,	Refl.REFR),//Glas
	Sphere(600, Vec(50,681.6-.27,81.6),	Vec(12,12,12),	Vec(),				Refl.DIFF) //Lite
];
...
void main(string[] args)
{
	...
	Ray cam = Ray(Vec(50,52,295.6), Vec(0,-0.042612,-1).norm()); // cam pos, dir
